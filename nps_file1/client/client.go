...
retry:
	retryNum++
	if retryNum < 3 {
		retryDuration = 3*60 + rand.Intn(30)
	} else if retryNum >= 3 {
		retryDuration = 10*60 + rand.Intn(30)
	} else if retryNum >= 5 {
		retryDuration = 15*60 + rand.Intn(30)
	} else {
		retryDuration = 30*60 + rand.Intn(60)
	}
	if CloseClient {
		return
	}
	...
	if err != nil {
		if strings.HasPrefix(err.Error(), "{[stop]}") {
			...
			logs.Error("The connection server failed and will be reconnected in %d Hour, error %s", randomRetry, err.Error())
