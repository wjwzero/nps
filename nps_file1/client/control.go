...
func handleRemoteLocalTcp(localAddr, rAddr, md5Password, role string, targetPort string) (remoteAddress string, err error) {
	...
	//	return
	//}
	logs.Info("Get remoteLocalAddr1 %s, remoteLocalAddr2 %s, remoteLocalAddr3 %s", remoteLocalAddr1, remoteLocalAddr2, remoteLocalAddr3)
	arr := strings.Split(remoteLocalAddr1, ":")
	remoteAddress = arr[0] + ":" + targetPort
	return
}
