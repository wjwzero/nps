...
func NewMapSplit(charMap []string, splitStr string) (m Map) {
	m.points = make([][]_Point, len(charMap))
	m.blocks = make(map[string]*_Point, len(charMap)*2)
	for x, row := range charMap {
		cols := strings.Split(row, splitStr)
		m.points[x] = make([]_Point, len(cols))
		for y, view := range cols {
			m.points[x][y] = _Point{x, y, view}
			if view == "X" {
...
func (this *Map) PrintMapArr(path *SearchRoad) (rowArr []string) {
...
func main() {
	...
	x := goTarget(presetMap, 0, 0, 0, 25, " ")
	x1 := goTarget(x, 0, 26, 18, 22, "")
	x2 := goTarget(x1, 18, 22, 18, 5, "")
	x3 := goTarget(x2, 18, 5, 1, 0, "")
	//m1 := NewMap(x)
	//searchRoad1 := NewSearchRoad(18, 25, 0, 0, &m1)
	//searchRoad1.FindoutRoad()
	//x1 := m1.PrintMapArr(searchRoad1)
	for _, a := range x3 {
		println(a)
	}
}

func goTarget(presetMap []string, startX int, startY int, endX int, endY int, splitStr string) []string {
	m := NewMapSplit(presetMap, splitStr)
	searchRoad := NewSearchRoad(startX, startY, endX, endY, &m)
	searchRoad.FindoutRoad()
	x := m.PrintMapArr(searchRoad)
	return x
}
