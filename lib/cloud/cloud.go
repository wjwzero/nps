...
retry:
	...
	if err != nil {
		...
		if retryNum < 3 {
			...
			retryDuration = 50 + rand.Intn(10)
		} else if retryNum >= 5 {
			retryDuration = 170 + rand.Intn(10)
		} else {
...
func getNpsNodeExternalIp(cloudAddr string, deviceKey string) (ip string, err error) {
	var data CommonData
	resp, err := http.Get(cloudAddr + "/xxxDispatch/ip?deviceKey=" + deviceKey)
	if err != nil {
		return "", err
	}
...
func CheckPassword(cloudAddr string, vKey string, password string) (res bool, err error) {
	...
	resp, err := http.Get(cloudAddr + "/xxxUser/verify" + password + "&deviceKey=" + vKey + "&authString=" + sign)
	if err != nil {
		return false, err
	}
	defer resp.Body.Close()
...
func CheckDeviceKey(cloudAddr string, deviceKey string) (res bool, err error) {
	sign := crypt.CreateSign([]string{deviceKey})
	var data CommonData
	resp, err := http.Get(cloudAddr + "/xxxDevice/verifyDeviceKey" + deviceKey + "&authString=" + sign)
	if err != nil {
		return false, err
	}
	defer resp.Body.Close()
